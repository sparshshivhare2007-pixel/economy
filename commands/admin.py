# commands/admin.py
# RyanBaka -> adapted for your economy bot
import html
import sys
import os
...
from helpers.utils import (
    SUDO_USERS, get_mention, resolve_target, format_money, reload_sudoers, stylize_text
)
from database.users import users, groups_db, sudoers, chatbot, get_user

# Owner & repo tokens loaded from env (optional)
...
async def update_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not UPSTREAM_REPO:
        return await update.message.reply_text("❌ <b>UPSTREAM_REPO</b> missing!", parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        return await msg.edit_text("❌ <b>Git Error:</b> Library missing.", parse_mode=ParseMode.HTML)
    except Exception as e:
        return await msg.edit_text(f"❌ <b>Git Error:</b> <code>{e}</code>", parse_mode=ParseMode.HTML)
...
async def sudolist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    owner_doc = users.find_one({"user_id": OWNER_ID})
    if not owner_doc:
        try:
    ...
    # SUDO_USERS might be a list of ints
    for uid in SUDO_USERS:
        if uid == OWNER_ID:
            continue
        u_doc = users.find_one({"user_id": uid})
        if u_doc:
...
def get_kb(act, arg):
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ 𝐘𝐞𝐬", callback_data=f"cnf|{act}|{arg}"),
                                  InlineKeyboardButton("❌ 𝐍𝐨", callback_data="cnf|cancel|0")]])


async def ask(update: Update, text, act, arg):
    await update.message.reply_text(f"⚠️ <b>Wait!</b> {text}\nAre you sure?", parse_mode=ParseMode.HTML,
...
async def addsudo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target:
        return await update.message.reply_text(err or "Usage: /addsudo <target>", parse_mode=ParseMode.HTML)
    if target['user_id'] in SUDO_USERS:
        return await update.message.reply_text("⚠️ Already Sudoer.", parse_mode=ParseMode.HTML)
...
async def rmsudo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target, err = await resolve_target(update, context, specific_arg=target_arg)
    if not target:
        return await update.message.reply_text(err or "Usage: /rmsudo <target>", parse_mode=ParseMode.HTML)
    if target['user_id'] not in SUDO_USERS:
        return await update.message.reply_text("⚠️ Not a Sudoer.", parse_mode=ParseMode.HTML)
...
async def addcoins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in SUDO_USERS:
        return
    if not context.args:
        return await update.message.reply_text("⚠️ Usage: <code>/addcoins 100 @user</code>", parse_mode=ParseMode.HTML)
    ...
    if not target:
        return await update.message.reply_text(err or "⚠️ Reply or Tag user.", parse_mode=ParseMode.HTML)
    await ask(update, f"Give <b>{format_money(amount)}</b> to {get_mention(target)}?", "addcoins",
...
async def rmcoins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in SUDO_USERS:
        return
    if not context.args:
        return await update.message.reply_text("⚠️ Usage: <code>/rmcoins 100 @user</code>", parse_mode=ParseMode.HTML)
    amount, target_str = parse_amount_and_target(context.args)
    if amount is None:
        return await update.message.reply_text("⚠️ Invalid Amount!", parse_mode=ParseMode.HTML)
    target, err = await resolve_target(update, context, specific_arg=target_str)
    if not target:
        return await update.message.reply_text(err or "⚠️ Reply or Tag user.", parse_mode=ParseMode.HTML)
    await ask(update, f"Remove <b>{format_money(amount)}</b> from {get_mention(target)}?", "rmcoins",
              f"{target['user_id']}|{amount}")
...
async def freerevive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target, err = await resolve_target(update, context, specific_arg=target_arg)
    if not target:
        return await update.message.reply_text(err or "Usage: /freerevive <target>", parse_mode=ParseMode.HTML)
...
async def unprotect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target, err = await resolve_target(update, context, specific_arg=target_arg)
    if not target:
        return await update.message.reply_text(err or "Usage: /unprotect <target>", parse_mode=ParseMode.HTML)
...
async def confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.from_user.id not in SUDO_USERS:
        return await q.message.edit_text("❌ <b>Baka!</b> Not for you.", parse_mode=ParseMode.HTML)

    data = q.data.split("|")
    act = data[1]
    if act == "cancel":
        return await q.message.edit_text("❌ <b>Cancelled.</b>", parse_mode=ParseMode.HTML)

    if act == "addsudo":
        uid = int(data[2])
        sudoers.insert_one({"user_id": uid})
        reload_sudoers(users)  # pass db/collection wrapper if needed
        await q.message.edit_text(f"✅ User <code>{uid}</code> promoted.", parse_mode=ParseMode.HTML)

    elif act == "rmsudo":
        uid = int(data[2])
        sudoers.delete_one({"user_id": uid})
        reload_sudoers(users)
        ...
        uid = int(data[2])
        users.update_one({"user_id": uid}, {"$set": {"status": "alive", "death_time": None}})
        await q.message.edit_text(f"✅ User <code>{uid}</code> revived.", parse_mode=ParseMode.HTML)

    elif act == "unprotect":
        uid = int(data[2])
        users.update_one({"user_id": uid}, {"$set": {"protection_until": datetime.utcnow()}})
